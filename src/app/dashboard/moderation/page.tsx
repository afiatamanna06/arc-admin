...
export default function ModerationPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div className="dark:text-white font-mono font-bold">DESIGN_MODERATION</div>
        <div className="text-xs text-teal-500 dark:text-teal-400 font-mono">APPROVE_REJECT_BRAND</div>
      </div>
        ...
        <div className="overflow-auto">
          <table className="min-w-full">
            <thead className="bg-white/5">
              <tr className="text-left text-xs text-teal-400 font-mono">
            ...
            <tbody>
              {designs.map((d, i) => (
                ...
                >
                  <td className="px-4 py-3 text-sm text-gray-500 dark:text-gray-400 font-mono">{d.id}</td>
                  ...
                  <td className="px-4 py-3 text-sm text-gray-500 dark:text-gray-400 font-mono">{d.author}</td>
                  ...
                  <td className="px-4 py-3 text-sm text-gray-500 dark:text-gray-400 font-mono">
                    <label className="inline-flex items-center gap-2 cursor-pointer">
                      <input
                        ...
                        className="w-4 h-4 dark:bg-black/50 border border-gray-600 text-teal-500"
                      />
                  ...
                  <td className="px-4 py-3 text-sm">
                    <input
                      ...
                      placeholder="Comment..."
                      className="w-full dark:bg-black/40 border border-gray-600 dark:text-white px-3 py-2 font-mono placeholder-gray-500"
                    />
                  </td>
                  <td className="px-4 py-3 text-sm text-right">
                    <div className="inline-flex items-center gap-2">
                      <button
                        onClick={() => setViewer({ open: true, src: d.imageUrl, title: d.title })}
                        className="inline-flex items-center gap-2 px-3 py-2 dark:bg-black/40 border border-teal-500/30 text-teal-300 font-mono text-xs hover:border-teal-400"
                      >
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-2 px-3 py-2 bg-green-500/20 border border-green-500/30 text-green-400 font-mono text-xs hover:border-green-400"
                      >
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-2 px-3 py-2 bg-red-500/20 border border-red-500/30 text-red-400 font-mono text-xs hover:border-red-400"
                      >
                        <X className="h-4 w-4" />
                        REJECT
                      </button>
        ...
        >
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2 dark:text-white font-mono">
                <BadgeCheck className="h-5 w-5 text-teal-300" />
                {viewer.title}
              </div>
              <button
                onClick={() => setViewer({ open: false })}
                className="px-3 py-2 bg-black/40 border border-gray-600 dark:text-white font-mono text-xs"
              >
                CLOSE
              </button>
            </div>
